...
def find_possible_glaciers(gdir, y0, ye, n, ex_mod=None, mb_offset=0, delete=False):
    ...
    path = os.path.join(gdir.dir, 'result'.join(str(y0)).join(cfg.PARAMS['baseline_climate']).join('_qc').join(str(cfg.PARAMS['climate_qc_months'])).join('_').join(str(cfg.PARAMS['prcp_scaling_factor'])).join('.pkl'))
